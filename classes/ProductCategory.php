...
class ProductCategoryManager extends DBManager
{
  ...
  {
    return $this->db->query("SELECT DISTINCT id, name, description, small_description, price, img FROM productcategory WHERE MATCH (name) AGAINST ('$search' IN NATURAL LANGUAGE MODE)
                             UNION SELECT DISTINCT id, name, description, small_description, price, img FROM productcategory WHERE MATCH (category) AGAINST ('$search' IN NATURAL LANGUAGE MODE)");
  }
